...
///
/// This function retrieves the `setup` and `update` Lua functions, injects a
...
pub(crate) fn game_loop(lua: &Lua) -> Result<(), String> {
    ...
    let setup: LuaFunction = retrieves_lua_loop_function(lua, "setup")?;
    let update: LuaFunction = retrieves_lua_loop_function(lua, "update")?;

    inject_loop_stopper(lua, &is_game_loop_active)?;

    if setup.call::<()>(()).is_err() {
        return Err("Failed to setup game loop.".to_string());
    }
